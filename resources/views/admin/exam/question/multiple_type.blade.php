  ...
  <div class="input_fields_wrap"> 
    @foreach ([0,1,2,3] as $key=>$value)
         <div id="div_{{ $key+1 }}">
            <input type="checkbox" id="answer" name="correct_answer[]" value="{{ $key+1 }}"> 
   ...
   <div class="input_fields_wrap"> 
    @foreach ($question['options'] as $key=>$option) 
           <div id="div_{{ $key+1 }}">
            <input type="checkbox" id="answer" name="correct_answer[]" value="{{ $key+1 }}" {{ $option['is_correct_ans']==1?'checked':'' }}> 
            ...
            <div> <textarea class="ckeditor" id="option_{{ $key+1 }}" name="option[]">{{ $option['description'] }}</textarea>
            </div> 
            <br>
          </div> 
      @endforeach
   ...
   <div class="input_fields_wrap"> 
    @foreach ($question['options'] as $key=>$value) 
           <div id="div_{{ $key+1 }}">
            @if (empty($question['is_correct_ans']))
             <input type="checkbox" id="answer" name="correct_answer[]" value="{{ $key+1 }}">
             @else
             <input type="checkbox" id="answer" name="correct_answer[]" value="{{ $key+1 }}" {{ in_array($key+1, $question['is_correct_ans'])?'checked':'' }}>
            @endif            
            <label> {{ $key+1 }}. Correct Answer</label>
            <label style="padding-left:10px"> Marking</label>
  ...
  $(document).ready(function() { 
    ...
    $(add_button).click(function(e){ //on add input button click
        ...
        if(x < max_fields){ //max input box allowed
            ...
            var div_x = 'div_' +x;
            $(wrapper).append('<div id="'+div_x+'"> <input type="checkbox" id="answer" name="correct_answer[]" value="'+x+'">  <label> '+x+'. Correct Answer</label>  <label style="padding-left:10px"> Marking</label> <input type="number" name="marking[]" style="width: 3em"><textarea id="'+editorId+'" class="ckeditor" name="option[]"></textarea></div></br>'); //add input box           
